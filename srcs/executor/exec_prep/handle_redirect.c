...
/*   Updated: 2022/10/18 16:49:13 by ychibani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	less_redirection(t_cmd_list *cmd_list)
{
	...
	if (cmd_list->redirection[0] < 0)
		return (print_error(cmd_list->redirect->file_name, "no such file or directory", NULL), 0);
	if (dup2(cmd_list->redirection[0], STDIN_FILENO) < 0)
		return (print_error(cmd_list->redirect->file_name, "can't dup file :", NULL), 0);
	if (close(cmd_list->redirection[0]))
		return (print_error(cmd_list->redirect->file_name, "can't close file :", NULL), 0);
	return (1);
}

int	great_redirection(t_cmd_list *cmd_list)
{
...
int	dgreat_redirection(t_cmd_list *cmd_list)
{
	if (cmd_list->redirection[1] != -1)
		close(cmd_list->redirection[1]);
	if (!test_access(cmd_list))	
...
int	hd_redirection(t_cmd_list *cmd_list)
{
	int		fd;
	char	*tmp_file;

	tmp_file = __get_name(cmd_list->index);
	if (!tmp_file)
		return (0);
	fd = open(tmp_file, O_RDWR | O_CREAT | O_APPEND, 0644);
	if (fd < 0)
		return (print_error(tmp_file, "no such file or directory", NULL), 0);
	if (close(fd) < 0)
		return (print_error(tmp_file, "can't close file :", NULL), free(tmp_file), 0);
	if (cmd_list->redirection[0] != -1)
		close(cmd_list->redirection[0]);
	if (!test_access(cmd_list))	
		return (0);
