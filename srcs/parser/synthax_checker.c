...
/*   Updated: 2022/10/10 17:20:05 by ychibani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (lexing->type == PIPE || lexing->type == REDIRECTION || lexing->type == HERE_DOC)
	{
		if (!lexing->next || lexing->next->type == REDIRECTION || lexing->next->type == NEW_LINE)
		{
			if (lexing->next && (lexing->next->type == REDIRECTION || lexing->next->type == HERE_DOC))
				return (1);
			data->rv = 2;
			data->synthax_error = SYNTAX_ERROR;
			return (0);
